...
public class NESemRelWithWikidata extends NESemRel_Abstract {
	...
	public NESemRelWithWikidata(RDFGraph kb, String wikidata_server) {
		this.setKb(kb);
...
//		System.out.println(n1.getURI().toString()+";"+n2.getURI().toString()+";"+count);
		...
		result = ((double)n1_count_comment + (double)n2_count_comment) * 1.0d +
					...
					((double)n1_wikiPageWikiLink + (double)n2_wikiPageWikiLink) * 0.8 +
					((double)n1_count_uri_dbo + (double)n2_count_uri_dbo) * 0.8 +
					((double)n1_seeAlso + (double)n2_seeAlso) * 1.0d +
					((double)n1_count_uri + (double)n2_count_uri) * 0.8 +
